...
### 1.6.3
- ⌘K 搜索浮层里的操作项现在位置更稳定：普通查询把网页搜索放在最前，完整网址把直接打开放在最前，并将网页搜索保留为底部兜底；搜索项还会显示当前引擎的名称和图标。
- 搜索框为空时的轮播提示现在会介绍 ⌘E 复制当前标签页快捷键。

### 1.6.2
- ⌘K 搜索浮层新增搜索引擎选择：点搜索框左侧的图标即可切换网页搜索用哪个引擎 —— 默认（浏览器默认引擎）、Google、Bing、GitHub、LinuxDo；选中「搜索 …」那条结果就用所选引擎搜索，选择会被记住。
- 搜索框为空时，占位提示每隔几秒轮播一条，介绍 Pounce 更多能力（直接打开网址、拼音搜中文、空格分隔多词筛选、在结果里关闭标签页），不再只有一句固定说明。

### 1.6.1
- 修复：1.6.0 在普通网页上按 ⌘K 打不开搜索浮层 —— 打包时漏掉了浮层要加载的一个脚本，更新到 1.6.1 即恢复。

### 1.6.0
- 全面暗色优先视觉重做：⌘K 搜索浮层、工具栏弹窗、设置页现在共用同一套以单一绿色为主调的设计（旧的浮层蓝色已退役），暗 / 亮双主题，默认跟随系统。
- 新增实验性 **标签分组**（在设置里手动开启）：同一网站开了多个标签时，会在搜索浮层里折叠成一行并显示数量；按 ↵ 展开成托盘、切换到任意标签，或一键关闭整组。
- 同一网址打开的多个标签现在各自独立显示与计数，不再被合并成一个。
- 新增「复制当前标签页」快捷键：⌘E（mac）/ Ctrl+Shift+E。
- 修复：在 Pounce 无法直接运行的页面（file://、view-source:、devtools://、chrome-untrusted://）上，会回退到一个可用标签页，而不是没有反应。

### 1.5.8
- 搜索框新增结果数量选择器 —— 可在底部切换 10 / 20 / 50 条，键盘也能操作。
- 修复：在使用 focus trap 的宿主弹窗（如 Quasar QDialog）中，⌘K 输入框不再被宿主页面抢走焦点。
- 结果图标的 favicon 渲染与宿主页面隔离，避免样式串扰；当页面 favicon 不可用时回退到安全地址。
